...
if __name__ == "__main__":
    ...
    results = [r.model_dump() for r in R.results]
    if results:
        df = pd.DataFrame(results)
        print(df.to_string(index=False))
        #
        assert list(df.columns) == ["Name", "ID", "Category", "Quantity_Available", "Unit_Price_AUD", "Total_Sales_AUD", "Last_Restock_Date", "Supplier"], "The columns name should match the specified format."
    else:
